...
describe("TranslationKey", () => {
  ...
  test("GET /translationKeys/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/translationKeys"}/${nonExistingId}`)
  ...
  test("GET /translationKeys/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/translationKeys"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
